document.addEventListener("DOMContentLoaded", () => {
  ...
  const tabFavoritesCountEl = document.getElementById("tabFavorites");
  ...
  function displayFlights(flights) {
    clearResults();
    if (flights.length === 0) {
      displayMessage("New flights to display", "info");
    }
  ...
  function createFlightCard(flight) {
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 pt-4 border-t border-gray-200">
        <div class="space-y-2">
          ...
          <p class="text-sm"><strong>Time</strong>${departure.scheduled} (Est:${formatTime(departure.estimated)})</p>
          <p class= "text-sm"><strong>Terminal/Gate</strong>${departure.terminal || "N/A"} / ${departure.gate || "N/A"}})</p>
          <p class= "text-sm"><strong>Delay:</strong><span class="${departure.delay ? "text-red-500 fo9nt-semibold" : ""}">${departure.delay ? `${departure.delay}min` : "No Delay"}</span></p>
        </div>
    </div>
